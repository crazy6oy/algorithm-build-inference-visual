...
def load_efficientnet(model, checkpoint_file, model_name):
    ...
    conversion_table = merge(
        conversion_table, conversion_table_for_weights_outside_blocks
    )
    ...
    conversion_table_for_first_block = {
        model._blocks[
            0
        ]._project_conv.weight: "blocks_0/conv2d/kernel",  # 1, 1, 32, 16]),
        model._blocks[
            0
        ]._depthwise_conv.weight: "blocks_0/depthwise_conv2d/depthwise_kernel",  # [3, 3, 32, 1]),
        model._blocks[0]._se_reduce.bias: "blocks_0/se/conv2d/bias",  # , [8]),
        model._blocks[
            0
        ]._se_reduce.weight: "blocks_0/se/conv2d/kernel",  # , [1, 1, 32, 8]),
        model._blocks[0]._se_expand.bias: "blocks_0/se/conv2d_1/bias",  # , [32]),
        model._blocks[
            0
        ]._se_expand.weight: "blocks_0/se/conv2d_1/kernel",  # , [1, 1, 8, 32]),
        model._blocks[0]._bn1.bias: "blocks_0/tpu_batch_normalization/beta",  # [32]),
        model._blocks[
            0
        ]._bn1.weight: "blocks_0/tpu_batch_normalization/gamma",  # [32]),
        model._blocks[
            0
        ]._bn1.running_mean: "blocks_0/tpu_batch_normalization/moving_mean",
        model._blocks[
            0
        ]._bn1.running_var: "blocks_0/tpu_batch_normalization/moving_variance",
        model._blocks[0]._bn2.bias: "blocks_0/tpu_batch_normalization_1/beta",  # [16]),
        model._blocks[
            0
        ]._bn2.weight: "blocks_0/tpu_batch_normalization_1/gamma",  # [16]),
        model._blocks[
            0
        ]._bn2.running_mean: "blocks_0/tpu_batch_normalization_1/moving_mean",
        model._blocks[
            0
        ]._bn2.running_var: "blocks_0/tpu_batch_normalization_1/moving_variance",
    }
    ...
    for i in range(len(model._blocks)):
        ...
        if is_first_block:
            conversion_table_block = {
                model._blocks[i]._project_conv.weight: "blocks_"
                + str(i)
                + "/conv2d/kernel",  # 1, 1, 32, 16]),
                model._blocks[i]._depthwise_conv.weight: "blocks_"
                + str(i)
                + "/depthwise_conv2d/depthwise_kernel",
                # [3, 3, 32, 1]),
                model._blocks[i]._se_reduce.bias: "blocks_"
                + str(i)
                + "/se/conv2d/bias",  # , [8]),
                model._blocks[i]._se_reduce.weight: "blocks_"
                + str(i)
                + "/se/conv2d/kernel",  # , [1, 1, 32, 8]),
                model._blocks[i]._se_expand.bias: "blocks_"
                + str(i)
                + "/se/conv2d_1/bias",  # , [32]),
                model._blocks[i]._se_expand.weight: "blocks_"
                + str(i)
                + "/se/conv2d_1/kernel",  # , [1, 1, 8, 32]),
                model._blocks[i]._bn1.bias: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/beta",  # [32]),
                model._blocks[i]._bn1.weight: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/gamma",  # [32]),
                model._blocks[i]._bn1.running_mean: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/moving_mean",
                model._blocks[i]._bn1.running_var: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/moving_variance",
                model._blocks[i]._bn2.bias: "blocks_"
                + str(i)
                + "/tpu_batch_normalization_1/beta",  # [16]),
                model._blocks[i]._bn2.weight: "blocks_"
                + str(i)
                + "/tpu_batch_normalization_1/gamma",  # [16]),
                model._blocks[i]._bn2.running_mean: "blocks_"
                + str(i)
                + "/tpu_batch_normalization_1/moving_mean",
                model._blocks[i]._bn2.running_var: "blocks_"
                + str(i)
            ...
            conversion_table_block = {
                model._blocks[i]._expand_conv.weight: "blocks_"
                + str(i)
                + "/conv2d/kernel",
                model._blocks[i]._project_conv.weight: "blocks_"
                + str(i)
                ...
                + str(i)
                + "/se/conv2d_1/kernel",
                model._blocks[i]._bn0.bias: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/beta",
                model._blocks[i]._bn0.weight: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/gamma",
                model._blocks[i]._bn0.running_mean: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/moving_mean",
                model._blocks[i]._bn0.running_var: "blocks_"
                + str(i)
                + "/tpu_batch_normalization/moving_variance",
                model._blocks[i]._bn1.bias: "blocks_"
                + str(i)
