...
def torch2tvm_module(
    torch_module: torch.nn.Module, torch_inputs: Tuple[torch.Tensor, ...], target
):
    torch_module.eval()
    input_names = []
    input_shapes = {}
...
def torch2executor(
    ...
    if (
        os.path.exists(lib_fname)
        and os.path.exists(graph_fname)
        and os.path.exists(params_fname)
    ):
...
def get_executor(use_gpu=True):
    ...
    if not os.path.exists(
        ...
        url = (
            "https://file.lzhu.me/projects/tsm/models/mobilenetv2_jester_online.pth.tar"
        )
