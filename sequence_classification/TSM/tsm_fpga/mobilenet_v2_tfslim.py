...
from nets.mobilenet.conv_blocks import (
    _fixed_padding,
...
def torch_to_conv_state(module, start_idx, depthwise=False):
    ...
    conv2d_init = {
        ...
        "weights_initializer": init(
            trans_w(state[conv_num + ".weight"].numpy()), verify_shape=True
        ),
        ...
        "normalizer_params": {
            ...
            "param_initializers": {
                "moving_mean": init(
                    state[bn_num + ".running_mean"].numpy(), verify_shape=True
                ),
                "moving_variance": init(
                    state[bn_num + ".running_var"].numpy(), verify_shape=True
                ),
...
def import_pytorch_weights():
    ...
    if not os.path.exists(
        "../online_demo/mobilenetv2_jester_online.pth.tar"
    ):  # checkpoint not downloaded
        print("Please run pytorch TSM online to download the model")
        return False

    torch_model.load_state_dict(
        torch.load("../online_demo/mobilenetv2_jester_online.pth.tar")
    )
    # torch_model.eval()

    i = 0
    for name, module in torch_model.named_modules():
        if isinstance(module, tsm_online.conv_bn) or isinstance(
            module, tsm_online.conv_1x1_bn
        ):
            ...
            i += 1
        elif isinstance(module, tsm_online.InvertedResidual) or isinstance(
            module, tsm_online.InvertedResidualWithShift
        ):
            print("BOTTLENECK")
            layers = module.conv
            ...
            for conv_idx in range(0, len(layers), 3):
                print(
                    names[name_idx]
                    + ": "
                    + str(layers.state_dict()[str(conv_idx) + ".weight"].numpy().shape)
                )
                params[names[name_idx]] = torch_to_conv_state(
                    layers, conv_idx, name_idx == 1
                )
                name_idx += 1
            torch_params_list.append(params)
            i += 1
...
V2_DEF_TSM = dict(
    ...
    spec=[
        op(
            slim.conv2d, stride=2, num_outputs=32, kernel_size=[3, 3], **torch_params(0)
        ),
        op(
            ops.expanded_conv,
        ...
        op(
            ops.expanded_conv_shift,
            stride=1,
            num_outputs=24,
            shift_buffer_name="shift_buffer_0:0",
            **torch_params(3),
        ),  # Shift
        ...
        op(
            ...
            stride=1,
            num_outputs=32,
            shift_buffer_name="shift_buffer_1:0",
            **torch_params(5),
        ),  # Shift
        op(
            ops.expanded_conv_shift,
            stride=1,
            num_outputs=32,
            shift_buffer_name="shift_buffer_2:0",
            **torch_params(6),
        ),  # Shift
        ...
        op(
            ...
            stride=1,
            num_outputs=64,
            shift_buffer_name="shift_buffer_3:0",
            **torch_params(8),
        ),  # Shift
        op(
            ops.expanded_conv_shift,
            stride=1,
            num_outputs=64,
            shift_buffer_name="shift_buffer_4:0",
            **torch_params(9),
        ),  # Shift
        op(
            ops.expanded_conv_shift,
            stride=1,
            num_outputs=64,
            shift_buffer_name="shift_buffer_5:0",
            **torch_params(10),
        ),  # Shift
        ...
        op(
            ...
            stride=1,
            num_outputs=96,
            shift_buffer_name="shift_buffer_6:0",
            **torch_params(12),
        ),  # Shift
        op(
            ops.expanded_conv_shift,
            stride=1,
            num_outputs=96,
            shift_buffer_name="shift_buffer_7:0",
            **torch_params(13),
        ),  # Shift
        ...
        op(
            ...
            stride=1,
            num_outputs=160,
            shift_buffer_name="shift_buffer_8:0",
            **torch_params(15),
        ),  # Shift
        op(
            ops.expanded_conv_shift,
            stride=1,
            num_outputs=160,
            shift_buffer_name="shift_buffer_9:0",
            **torch_params(16),
        ),  # Shift
...
shift_buffer = [
    tf.placeholder(tf.float32, shape=shift_buffer_shapes[0], name="shift_buffer_0"),
...
with tf.Session() as sess:
    ...
    if DUMP_QUANTIZE:
        ...
        for op in graph.get_operations():
            ...
            shift_out_search = re.search(
                "shift(_(\d+))?/shift_split_buffer_output$", op.name
            )
        ...
        if QUANTIZE_LOCAL:
            ...
            img_paths = [
                ...
                for x in sorted(os.listdir(input_dir))[:IMAGENET_IMGS]
            ]
        dump_data = {}
        for img_num, p_img in enumerate(img_paths):
            ...
            for i in range(len(input_dict)):
                shift = (
                    outputs[shift_in_dict[i]]
                    if shift_in_dict[i] < len(outputs)
                    else np.empty(1)
                )
                resid = (
                    outputs[input_dict[i]]
                    if input_dict[i] < len(outputs)
                    else np.empty(1)
                )
                if img_num == 0:
                    dump_data[i] = {}
                dump_data[i][img_num] = {
                    "shift_concat": shift.tolist(),
                    "resid": resid.tolist(),
                }
    ...
    if EXPORT:
        ...
        tf.train.write_graph(
            graph_or_graph_def=input_graph_def,
            logdir=save_dir,
            name=pbtxt_file,
            as_text=True,
        )
        ...
        export_quantize_info(
            os.path.join(save_dir, "quantize_info.txt"), inputs, output_node_names
        )
    ...
    for split_num, out in enumerate(split_outputs):
        ...
        with tf.Session() as split_sess:
            ...
            for op in split_graph.get_operations():
                ...
                # shift buffer input
                if split_num > 0:
                    inputs[shift_buffer[split_num - 1]] = np.ones(
                        shift_buffer_shapes[split_num - 1]
                    )
                # CPU input for shift
                if op.type == "Placeholder" and "/input" in op.name:
                    inputs[op.name + ":0"] = np.ones(op.outputs[0].get_shape())
