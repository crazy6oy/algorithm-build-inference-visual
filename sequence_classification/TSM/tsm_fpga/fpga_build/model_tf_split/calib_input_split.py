...
quantize_info_path = os.path.join(
    CALIB_BASE_PATH, f"model_tf_split_{CALIB_MODEL_SPLIT}/quantize_info.txt"
)
input_info_path = os.path.join(
    CALIB_BASE_PATH, f"model_tf_split_{CALIB_MODEL_SPLIT}/inputs.pickle"
)
...
with open(quantize_info_path) as f:
    ...
    raw_input_shapes = [
        [int(x) for x in shape.split(",")] for shape in raw_input_shapes
    ]
