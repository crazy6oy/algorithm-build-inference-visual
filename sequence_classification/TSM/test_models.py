...
parser.add_argument(
    "--dense_sample", default=False, action="store_true", help="use dense sample as I3D"
)
parser.add_argument(
    "--twice_sample",
    default=False,
    action="store_true",
    help="use twice sample for ensemble",
)
parser.add_argument(
    "--full_res",
    default=False,
...
for this_weights, this_test_segments, test_file in zip(
    ...
    print(
        "=> shift: {}, shift_div: {}, shift_place: {}".format(
...
def eval_video(video_data, net, this_test_segments, modality):
    ...
    with torch.no_grad():
        ...
        if net.module.is_shift:
            ...
            rst = (
                rst.reshape((batch_size, -1, num_class))
                .mean(axis=1)
                .reshape((batch_size, num_class))
            )
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        for n_seg, (_, (data, label)), net, modality in zip(
            test_segments_list, data_label_pairs, net_list, modality_list
        ):
        ...
        prec1, prec5 = accuracy(
            torch.from_numpy(ensembled_predict), this_label, topk=(1, 5)
        )
        top1.update(prec1.item(), this_label.numel())
        top5.update(prec5.item(), this_label.numel())
        if i % 20 == 0:
            print(
                "video {} done, total {}/{}, average {:.3f} sec/video, "
