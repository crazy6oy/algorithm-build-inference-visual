...
def main():
    ...
    saveResult = True
    dictCategories = {"phase": ["0", "2", "7", "8", "9", "10", "11", "12", "13", "14"]}
    imageFolder = "/public/shoushu/gjy/wangyx/dataset/LPDFrame1FPS"
    sequenceJsonPath = "/public/shoushu/gjy/wangyx/project/LPDV4_9_test.json"
    recordJson = "./LPD9_5_2_2.json"
    num_class = 0
    for rule in dictCategories:
        num_class += len(dictCategories[rule])
    args.resume = "/public/shoushu/gjy/wangyx/project/TSMxx/TSMLPD/checkpoint/TSM_something_RGB_resnet50_shift8_blockres_avg_segment8_e120/ckpt.best.pth.tar"
    ...
    prec1 = validate(val_loader, model, dictCategories, recordJson, saveResult)


def validate(val_loader, model, dictCategories, recordJson, saveResult=False):
    ...
    with torch.no_grad():
        for i, (input, target, imagesName) in enumerate(tqdm.tqdm(val_loader)):
            ...
            outputT = output[:]
            targetT = target[:, 0]
            imagesNameT = [y for x, y in enumerate(imagesName)]
            ...
            if saveResult:
                for j, imageName in enumerate(imagesNameT):
                    ...
                    dictOutput[imageName][list(dictCategories.keys())[0]] = [
                        resultTemp[j].item(),
                        targetTemp[j].item(),
                        confTemp[j].item(),
                    ]
    ...
    if saveResult:
        with open(recordJson, "w", encoding="utf-8") as f:
            ...
            f.close()
    mAP = []
    print(list(dictCategories.keys())[0])
    ...
    aps = getConfusionMatrix(tp.numpy(), labels.numpy(), results.numpy(), conf.numpy())
    mAP.extend(aps)
