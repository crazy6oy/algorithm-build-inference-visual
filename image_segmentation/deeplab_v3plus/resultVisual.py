...
class Trainer(object):
    ...
    def test(self, save_result=False, save_folder="output"):
        self.model.eval()
        self.evaluator.reset()
        tbar = tqdm(self.test_loader, desc="\r")
        test_loss = 0.0
        for i, (sample, list_img_name) in enumerate(tbar):
            image, target = sample["image"], sample["label"]
            if self.args.cuda:
                image, target = image.cuda(), target.cuda()
            with torch.no_grad():
            ...
            if save_result:
                self.save_mask(pred, save_folder, list_img_name)
    ...
    # def save_mask(self, Nmask, save_folder, list_img_name):
    #     os.makedirs(save_folder, exist_ok=True)
    #     Nmask = Nmask.repeat(3, 1).astype(np.uint8)
    #     for i, img_name in enumerate(list_img_name):
    #         save_path = os.path.join(save_folder, img_name.replace(".jpg", ".png"))
    #         cv2.imwrite(save_path, Nmask[i])
    def save_mask(self, Nmask, save_folder, list_img_name):
        os.makedirs(save_folder, exist_ok=True)
        Nmask = np.expand_dims(Nmask, 3).repeat(3, 3).astype(np.uint8)
        for i, img_name in enumerate(list_img_name):
            save_path = os.path.join(save_folder, img_name.replace(".jpg", ".png"))
            cv2.imwrite(save_path, Nmask[i])
...
def main(gpu_ids):
    ...
    parser.add_argument(
        "--imageFolder",
        type=str,
        default=r"D:\work\dataSet\organSegmentation\for_train\dataset01",
        help="origin image & mask folder",
    )
    parser.add_argument(
        "--dataSetRecordPath", type=str, default=None, help="limit train file"
    )
    parser.add_argument("--is_train", type=str, default=False, help="train or test")
    ...
    parser.add_argument(
        "--resume",
        type=str,
        default=r"D:\work\dataSet\organSegmentation\checkPoint\v0\0\model_best.pth.tar",
